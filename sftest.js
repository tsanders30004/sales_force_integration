/*
    Bluefin Sales Force Integrations
...
/* 
    these are the queries to populate the in-memory tables from sales force; no logic here.  just getting data to work with.
    to add a new table, you only need to add an attribute that corresponds to the sales force object name with the relevant SELECT statement.
*/
var objQueries={
...
const fnSync=function(){
    ...
    */
    ...
    for(var i=0;i<arrTables.length;i++){
        if(!objData[arrTables[i]] || objData[arrTables[i]].length === 0){ // what does !objData[arrTables[i]] do?
...
*/
    ...
    /* 
        create the SELECT statement that will retrieve the columns necessary to reconstruct the existing account table.
        the question marks are placeholders for the tables that are used in a la sql.
        the tables are passed to a la sql in an array.
    */
    var strQuery = 
            'SELECT \
    ...
    /* 
        arrOutput is an array with the result set of the in-memory JOIN.
        question:  how large a result set is possible?  
        this set is 171 records.  what if the result set contains a million rows?
    */
    
    console.log('\ndegug_05 number of records\n' + arrOutput.length);
    console.log('\ndebug_06 arrOutput[0].acct_id\n' + arrOutput[0].acct_id);
    
    /* 
        save the output file.
        how do we know that the result set is ready before we execute this line?
    */
...
const fnQuery=function(strTable){
    /*
        purpose:  run the query for each object / table and 'populate the related data'...  ???
    */
    console.log('\ndebug_07 strTable\n' + strTable);
    sfConnect.query(objQueries[strTable], function(objError, objResponse) {
        if (objError){
            return console.error(objError); 
        }
        if(objResponse.records){ 
            objData[strTable]=fnConvert(strTable,objResponse.records); 
            if(strTable === 'Contract'){
                // console.log(objResponse.records); 
                fnSync(); 
            }
        }
        else{
            console.log('still waiting on records for '+ strTable)
        }
    });
};
